...
// The modal is now in ProductFormModal.jsx — this file owns list state + CRUD actions only.

import React, { useState, useEffect } from 'react';
import { Plus, Edit2, Trash2, Loader2, CheckCircle, XCircle, Tag, Search, ChevronLeft, ChevronRight } from 'lucide-react';
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden shadow-xl flex flex-col">
                  ...
                  <tr key={product.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-4 flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {isDiscounted && <Tag size={12} className="text-green-400" />}
                        </div>
                    ...
                    <td className="p-4">
                      {product.stock_count != null
                        ? <span>{product.stock_count} units</span>
          ...
          {isLoading ? (
            ...
            return (
              <div key={product.id} className="p-4 hover:bg-white/5 transition-colors flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3 bg-black/40 p-3 rounded-lg border border-white/5 text-sm">
                  <div>
                  ...
                  <div>
                    ...
                    <span>{product.stock_count != null ? `${product.stock_count} units` : 'Unlimited'}</span>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-0.5">Gender</p>
                    <span>{product.gender}</span>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-0.5">Status</p>
                    {product.available
        ...
        {!isLoading && totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 p-4 border-t border-white/10 bg-black/20">
            <button disabled={activePage === 1} onClick={() => setActivePage(p => p - 1)} className="p-2 border border-white/10 rounded hover:border-gold-400 text-gray-400 hover:text-gold-400 disabled:opacity-30 transition-colors"><ChevronLeft size={18} /></button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
              <button key={num} onClick={() => setActivePage(num)} className={`w-8 h-8 rounded text-sm font-bold transition-all ${activePage === num ? 'bg-gold-400 text-black' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>{num}</button>
            ))}
            <button disabled={activePage === totalPages} onClick={() => setActivePage(p => p + 1)} className="p-2 border border-white/10 rounded hover:border-gold-400 text-gray-400 hover:text-gold-400 disabled:opacity-30 transition-colors"><ChevronRight size={18} /></button>
          </div>
        )}
      </div>
