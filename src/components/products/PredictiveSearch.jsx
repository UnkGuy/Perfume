        ...
        <div className="absolute top-full left-0 w-full mt-2 bg-rich-black border border-gold-400/30 rounded-xl shadow-2xl overflow-hidden animate-slide-in">
          {searchSuggestions.map(suggestion => (
            <div 
              ...
              className={`px-4 sm:px-6 py-4 hover:bg-white/5 cursor-pointer flex justify-between items-center gap-4 border-b border-white/5 last:border-0 transition-colors w-full min-w-0 ${!suggestion.available ? 'opacity-70' : ''}`}
            >
              <div className="min-w-0 flex-1">
                <span className="font-bold text-white block truncate">{suggestion.name}</span>
                <span className="text-xs text-gray-400 block truncate">{suggestion.brand}</span>
              </div>
              <div className="flex flex-col items-end flex-shrink-0">
                <span className="text-gold-400 font-medium whitespace-nowrap">₱{suggestion.price}</span>
                {!suggestion.available && <span className="text-[10px] text-red-400 font-bold tracking-widest mt-1 whitespace-nowrap">OUT OF STOCK</span>}
              </div>
            </div>
          ))}
        </div>
