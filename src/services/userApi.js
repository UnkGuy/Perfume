...
export const toggleUserBanAPI = async (userId, isBanned) => {
  const { data, error } = await supabase.from('profiles').update({ is_banned: isBanned }).eq('id', userId).select();
  if (error) throw error;
  if (data && data.length === 0) {
    const { error: insertError } = await supabase.from('profiles').insert([{ id: userId, is_banned: isBanned }]);
